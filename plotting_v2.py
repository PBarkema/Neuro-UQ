...
# Scale using a new scaler to allow equal contribution to the PEPI inv/val plot
sc_PE_inv_mean, sc_PE_inv_up, sc_PE_inv_lo, pe_inv_scaler = scale_bounds(sm_PE_inv_mean, sm_PE_inv_up, sm_PE_inv_lo)
sc_PE_val_mean = pe_inv_scaler .transform(sm_PE_val_mean.reshape(-1,1)).flatten()
sc_PE_val_up = pe_inv_scaler .transform(sm_PE_val_up.reshape(-1,1)).flatten()
sc_PE_val_lo = pe_inv_scaler .transform(sm_PE_val_lo.reshape(-1,1)).flatten()
...
# Scale using a new scaler to allow equal contribution to the PEPI inv/val plot
sc_P_val_mean, sc_P_val_up, sc_P_val_lo, p_val_scaler = scale_bounds(sm_P_val_mean, sm_P_val_up, sm_P_val_lo)
sc_P_inv_mean = p_val_scaler.transform(sm_P_inv_mean.reshape(-1,1)).flatten()
sc_P_inv_up = p_val_scaler.transform(sm_P_inv_up.reshape(-1,1)).flatten()
sc_P_inv_lo = p_val_scaler.transform(sm_P_inv_lo.reshape(-1,1)).flatten()

# E. Difference (Plot 7)
# Hippocampus plot
PE_U_P_I_history = PE_Uhat_history + P_I_history
PE_U_P_I = np.nanmean(PE_U_P_I_history, axis=0)
PE_U_P_I_se = get_se(PE_U_P_I_history)
sm_PE_U_P_I_mean, sm_PE_U_P_I_upper, sm_PE_U_P_I_lower = get_smooth_bounds(PE_U_P_I, PE_U_P_I_se, wdw)
PE_U_P_I = sc_PE_mean + sc_P_mean # The main line
# Difference Valid/Invalid SEs
PE_U_P_I_se_invalid = PE_U_P_I_se[P_I_invalid_indices] # Assuming indices align
PE_U_P_I_se_valid = PE_U_P_I_se[P_I_valid_indices]
...
PE_U_P_I_upper = PE_U_P_I + combined_se_scaled
PE_U_P_I_lower = PE_U_P_I - combined_se_scaled

# F. Difference Valid/Invalid (Plot 8)
# Using raw differences of smoothed values as per original code
PE_U_P_I_inv_mean = sc_PE_inv_mean + sc_P_inv_mean
PE_U_P_I_val_mean = sc_PE_val_mean + sc_P_val_mean
# Approx SE
se_PE_U_P_I_inv = np.sqrt((sc_PE_inv_up - sc_PE_inv_mean)**2 + (sc_P_inv_up - sc_P_inv_mean)**2)
se_PE_U_P_I_val = np.sqrt((sc_PE_val_up - sc_PE_val_mean)**2 + (sc_P_val_up - sc_P_val_mean)**2)
...
PE_V_ax.plot(PE_Uhat_invalid_indices, sc_PE_inv_mean, c=red) #sm_PE_inv_mean
PE_V_ax.fill_between(PE_Uhat_invalid_indices, sc_PE_inv_lo, sc_PE_inv_up, color=red, alpha=0.2)
PE_V_ax.plot(PE_Uhat_valid_indices, sc_PE_val_mean, c=green)
PE_V_ax.fill_between(PE_Uhat_valid_indices, sc_PE_val_lo, sc_PE_val_up, color=green, alpha=0.2)
...
P_V_ax.plot(P_I_invalid_indices, sc_P_inv_mean, c=red)
P_V_ax.fill_between(P_I_invalid_indices, sc_P_inv_lo, sc_P_inv_up, color=red, alpha=0.2)
P_V_ax.plot(P_I_valid_indices, sc_P_val_mean, c=green)
P_V_ax.fill_between(P_I_valid_indices, sc_P_val_lo, sc_P_val_up, color=green, alpha=0.2)
...
P_PE_ax.fill_between(trials, PE_U_P_I_lower, PE_U_P_I_upper, color=blue, alpha=0.2)
...
P_PE_V_ax.plot(P_I_invalid_indices, PE_U_P_I_inv_mean, c=red)
P_PE_V_ax.fill_between(P_I_invalid_indices, PE_U_P_I_inv_mean - se_PE_U_P_I_inv, PE_U_P_I_inv_mean + se_PE_U_P_I_inv, color=red, alpha=0.2)
P_PE_V_ax.plot(P_I_valid_indices, PE_U_P_I_val_mean, c=green)
P_PE_V_ax.fill_between(P_I_valid_indices, PE_U_P_I_val_mean - se_PE_U_P_I_val, PE_U_P_I_val_mean + se_PE_U_P_I_val, color=green, alpha=0.2)
